...
- The encrypted row-oriented SQLite mail index uses WAL transactions and UID indexes for bounded page/exact-message reads. List pages read a separate encrypted body-free summary table, so their B-tree never traverses multi-megabyte cached-body overflow pages or transfers bodies through IPC; schema-v3 migration builds missing summaries in throttled 48-row background batches. A random cache master key is unlocked from the operating-system credential store once per process, while each row uses its own XChaCha20-Poly1305 nonce; legacy Windows DPAPI-per-row ciphertext remains readable and migrates after first-paint in bounded background batches. Transactional folder counts remove repeated full-folder `COUNT(*)` reads, and first/subsequent page SQL uses direct primary-key ranges. Completed one-shot IMAP commands return after their tagged server response instead of waiting up to the socket timeout for a best-effort `LOGOUT`; one bounded background worker performs a graceful close while concurrent completions drop their TLS stream immediately. Synchronization returns as soon as mail is durable, and a separate rate-limited process-wide single-flight worker creates the online recovery copy without holding the user-visible path; confirmed database corruption is quarantined and restored from an integrity-checked backup, while lock, permission, and disk errors are never misclassified as corruption.
...
- RFC `Message-ID`, `In-Reply-To`, and `References` metadata is parsed through bounded native fields and grouped with a folder/account-scoped reply graph. The virtual list renders one row per conversation with participant, unread, and message counts; selecting a conversation maps back to its individual messages. The reading view fetches the selected body immediately after first paint, shows an isolated progress strip, and then warms only the two adjacent rows on either side through one idle request at a time; in-flight requests are deduplicated and offline-only mode performs no remote read-ahead. Foreground reads use account-isolated pooled IMAP lanes capped at eight accounts, with active-slot protection, five-minute idle pruning, and least-recently-used eviction; speculative read-ahead remains confined to one separately reaped process-wide lane. Reply and reply-all compose flows emit bounded RFC reply headers, preserving them through encrypted drafts and the offline outbox so online delivery and later retries stay in the same provider conversation.
...
- Safe HTML preview mode with a restart-persistent 70–100% mail-body scale (80% desktop default), bounded descendant typography, non-expanding nested tables, an explicit anti-phishing external-link confirmation, attachments, smart categories (including Apple Connect and Apple advertising), search, unread filter, star, reply, compose, theme switching, and user CSS overrides. Resizable Windows layouts keep the compact three-pane desktop workspace down to 721px and switch to single-pane navigation only below that width. Link confirmation prominently shows the actual hostname or mail recipient, warns about display-target mismatches, Punycode, IP destinations, non-standard ports, and multiple recipients, and never prefetches the destination.
...
- Encrypted local sender/domain blocking rules support exact senders, domain subtrees, per-account scope, and global scope. Matches disappear immediately from ordinary folders but remain inspectable under “推广”; blocked rows are excluded from unread badges, new-mail tray notifications, and snooze-due notification counts without rewriting the underlying encrypted mailbox cache.
...
- Native sync discovers selectable IMAP mailboxes (including custom folders), persists a bounded per-account folder index, and exposes those folders in the desktop sidebar for offline browsing and UID pagination. RFC 6154 special-use attributes map localized sent, drafts, junk, trash, archive, and all-mail folders to their semantic desktop destinations; provider-name fallbacks are accepted only when the server actually lists them, so startup no longer waits on sequential probes of nonexistent default folders.
...
- Compose supports real scheduled sending with quick evening/morning/next-Monday choices and a bounded custom local time. Scheduled messages enter the encrypted outbox immediately, survive restart, sleep, offline periods, and close-to-tray, and reuse the single background scheduler at the requested time. The outbox distinguishes user schedules from the short undo-send window; editing recalls the complete draft, while “立即发送” deliberately clears the schedule.
- Messages can be moved into a local encrypted “稍后处理” folder with quick or custom reminder times. Only a body-free bounded summary and the native account/folder/UID identity are copied; one condition-variable scheduler wakes at the exact next deadline, returns due mail to the inbox even in offline-only mode, and emits a count-only Windows notification when notifications are enabled. Rescheduling deduplicates the same native message, account removal clears its reminders, and the 1,000-message/one-year limits prevent unbounded local state.
...
- Compose includes a compact WebView2-native rich-text surface for bold, italic, underline, ordered/unordered lists, quotations, safe links, and sanitized rich paste. Plain-text alternatives remain authoritative for compatibility; the matching safe HTML survives encrypted draft autosave, restart, offline outbox delivery, and recall-to-draft. Remote-image URLs, scripts, event handlers, forms, styles, and unsafe link schemes are stripped again at the Rust IPC boundary, while inline images remain CID-bound attachments.

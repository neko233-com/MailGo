...
const nativeMain = readFileSync(resolve(root, 'native/src/main.rs'), 'utf8')
const nativeSync = readFileSync(resolve(root, 'native/src/sync.rs'), 'utf8')
...
assert.match(app, /priority: MessageHydrationPriority = 'foreground'/)
assert.match(app, /priority,/)
assert.match(app, /messageHydrationRef\.current\.get\(mail\.id\) === request/)
assert.match(app, /}, 1_000\),/)
assert.match(app, /requestNativeMessage\(candidate, isForeground \? 'foreground' : 'read-ahead'\)/)
assert.match(nativeMain, /BodyFetchPriority::parse/)
assert.match(nativeMain, /Some\(Value::String\(value\)\)/)
assert.match(nativeMain, /invalid message fetch priority/)
assert.match(nativeSync, /BodyFetchPriority::Foreground =>/)
assert.match(nativeSync, /BodyFetchPriority::ReadAhead =>/)
assert.match(nativeSync, /READ_AHEAD_BODY_SESSION/)
assert.match(nativeSync, /MAX_FOREGROUND_BODY_SESSION_ACCOUNTS: usize = 8/)
assert.match(nativeSync, /fn prune_idle_body_sessions\(\)/)
assert.match(nativeSync, /prune_idle_body_sessions\(\);/)
assert.match(nativeSync, /Arc::strong_count/)
assert.match(nativeSync, /fn evict_lru_body_session/)
...
console.log('Bounded priority-isolated body hydration, first-page reveal, and hot-path account index checks passed.')

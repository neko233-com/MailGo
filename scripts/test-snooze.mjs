...
const mailboxCounts = read('src/mailboxCounts.ts')
...
assert.match(mailboxCounts, /mail\.blocked \|\| mail\.snoozedUntil != null/)
assert.match(app, /displayedAccountUnreadCounts/)
assert.match(app, /account\.unread - \(mailboxCountIndex\.hiddenUnreadByAccount\.get\(account\.id\) \?\? 0\)/)

...
export type DisplayDensity = 'dense' | 'compact' | 'comfortable'
...
export function SettingsPopover({
  ...
  return <motion.div className="settings-popover" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 8 }}>
    ...
    <label className="settings-row settings-select-row"><span><Icon name="menu" size={17} /><span>界面密度<small>{displayDensity === 'dense' ? '极致紧凑 · 28px 邮件行' : displayDensity === 'compact' ? '紧凑 · 32px 邮件行' : viewportRequiresCompactDensity ? '窗口较小，已自动紧凑' : '舒适 · 更大间距'}</small></span></span><select aria-label="界面密度" value={displayDensity} onChange={(event) => onDensityChange(event.target.value as DisplayDensity)}><option value="dense">极致紧凑</option><option value="compact">紧凑</option><option value="comfortable">舒适</option></select></label>
